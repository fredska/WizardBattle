...
public class Player extends Entity {
	...
	public void update(float delta) {
		...
		endX = (int) ((this.getPosition().x) / this.WIDTH) + 2;
		...
		Rectangle boundingBox = new Rectangle(getPosition().x, getPosition().y,
				WIDTH, HEIGHT);
		...
		for (Rectangle tile : tiles) {
			...
			if (collisionDetector.broadPhaseCollisionCheck(boundingBox, tile,
					getVelocity(), delta)) {
				collisionDetected = true;
				// Make a copy of the current Velocity
				Vector2 tmpVelocity = getVelocity().cpy();
				// Get the time till collision
				timeToCollision = collisionDetector.nextIteration(boundingBox,
						tile, getVelocity(), delta);
				if (timeToCollision == 0.0f) {
					isNextToWall = true;
					continue;
				}
				// Move the player from the start time till delta with the given
				// velocity
				getPosition().add(timeToCollision * tmpVelocity.x,
						timeToCollision * tmpVelocity.y, 0);

				// Now, with the new velocity from the detector, move the player
				// the rest of the way
				getPosition().add((delta - timeToCollision) * getVelocity().x,
						(delta - timeToCollision) * getVelocity().y, 0);

			}
		}
		// Update the player's position if no collisions were detected;
		if (!collisionDetected || isNextToWall)
			getPosition().add(delta * getVelocity().x, delta * getVelocity().y,
					0);
	...
	public void debug(Camera camera, BaseLevel level) {
		...
		shapeRenderer.rect(startX * 32, startY * 32, 32 * (endX - startX),
				32 * (endY - startY));
		shapeRenderer.end();
	}
	...
	public void draw(Camera camera) {
		...
		shapeRenderer.rect(super.getPosition().x, super.getPosition().y,
